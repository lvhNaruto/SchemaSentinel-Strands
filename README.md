...
2. Select Topics from **Upstream Ingress Stream.**
...
4. Open **Judge Playground**, pick **Alien Payload**, click **Test Healing** — the real Strands
   agent synthesizes a patch, the AST sandbox verifies it, and the verdict stepper shows
   Detected → Agent → AST → Healed (or → DLQ).
5. Watch the warehouse fill with ✨ Auto-Healed rows and the DLQ stay clean.
